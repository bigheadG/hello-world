...
# JB library 2021.06.27

#
def jb_label_to_name(label, dict):
    # refer: https://numpy.org/doc/stable/reference/arrays.dtypes.html
    jb_labelName = np.empty([len(label)], dtype='U10') # use 'U' better
    for i in range(len(label)):
        jb_labelName[i] = dict[label[i]]
    return jb_labelName
...
def video_init(is_2_write=False,save_path=None):
    writer = None
    cap = cv2.VideoCapture(1) # 0 for localBuildCam, 1 for UsbCam
    print('JB> cap={}'.format(cap))
...
class Yolo_v4():
    ...
    def detection(self,img_bgr):
        ...
        #print("score:",score) # original

        #################################################################################################
        # show label name for easy reading
        jb_labelName = jb_label_to_name(label, self.label_dict)
        # show float array in two decimal places
        # refer: https://stackoverflow.com/questions/38492390/how-to-make-numpy-print-trailing-0s
        np.set_printoptions(formatter={'float': '{: 0.2f}'.format}) # for float array (score) print two decimal in places
        print("JB.> flow={}, score={}, name={}".format(self.JB_flow, score, jb_labelName))
        self.JB_flow += 1
        #################################################################################################
...
if __name__ == "__main__":
    ...
    model_path = r"YOLO_v4_416.ckpt.meta"
    GPU_ratio = None # full of resource # 0.4 original
